...
int main() {
    ...
    // Add a single file
    std::string testFile = "~/Downloads/corpus/current/0.System Design Interview An Insider’s Guide by Alex Xu.pdf";
    tldr_cpp_api::addCorpus(testFile);

    // Add a folder
    tldr_cpp_api::addCorpus("~/Downloads/corpus/current");

    // Do RAG
    std::string query = "What is the hotspot problem in cache?";
    std::string corpus_dir = "~/Downloads/corpus/current";
